...
export async function checkAndSync(force = false) {
    ...
    try {
        ...
        // ECONOMY CHECK: Skip if last sync was very recent
        if (!force) {
            ...
            if (meta.length > 0) {
                ...
                // If we are missing data for the current month, we check every 1 hour. 
                // If everything is up to date, we wait 4 hours for economy.
                const { rows: lastEntry } = await client.sql`SELECT MAX(date) as last_date FROM indicators`;
                const lastDate = lastEntry[0]?.last_date ? new Date(lastEntry[0].last_date) : null;
                const isMissingCurrentData = !lastDate || (lastDate.getUTCMonth() < (new Date().getUTCMonth() - 1));
                
                const waitTime = isMissingCurrentData ? 1 : 4;

                if (diffHours < waitTime) {
                    console.log(`[ECONOMY] Sync skipped. Last check was ${diffHours.toFixed(2)}h ago (Threshold: ${waitTime}h).`);
                    return false;
                }
            }
        }
