    ...
    for (const [name, code] of Object.entries(INDICATORS)) {
        try {
            ...
            if (lastDateStr) {
                ...
                // Go back 2 months from the last date to ensure we catch late updates or corrections
                const date = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, 1);
                date.setMonth(date.getMonth() - 1); 
                
                startDate = `01/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getFullYear()}`;
            } else {
                // Default history: 20 years for monthly indices
            ...
            console.log(`[SYNC] Inserting ${data.length} rows for ${name}...`);
